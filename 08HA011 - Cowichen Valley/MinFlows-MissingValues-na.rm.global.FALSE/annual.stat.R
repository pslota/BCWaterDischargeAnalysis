# Analysis of Streamflow data - what happens if there are missing values that are skipped
...
set.seed(3423432)
sum(is.na(flow[ start.year <= flow$Year & flow$Year <= end.year, "Q"]))

missing <- sample(1:nrow(flow), .0003*nrow(flow))
flow$Q[ missing ] <- NA
sum(is.na(flow[ start.year <= flow$Year & flow$Year <= end.year, "Q"]))
...
vfa.analysis <- compute.volume.frequency.analysis( Station.Code=Station.Code, flow, 
                      start.year=start.year.vfa, end.year=end.year.vfa, use.water.year=TRUE, 
