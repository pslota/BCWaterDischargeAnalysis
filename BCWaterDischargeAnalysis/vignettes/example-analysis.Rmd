...
```
Notice the defaut acton (see below) is to propogate missing values. Despite a complete data record starting
in 1965, there was no data available for 1964-12-31 which implies that a three-day rolling average that
includes 1964-12-31 cannot be computed which gives a missing values for the 3-day rolling average minimum
flow for 1965. This also occurs for other rolling averages that include any 1964 data such as water year computation
...
in the period of record, there are no missing summary statistics (see below for more details).
These statistics can be then processed as you desire.
...
Accoding to the WSC, 
> monthly or annual values are not calculated unless there is complete daily record. 
> If the gauge went down and the daily record cannot be reasonable estimated,
> then summaries at longer time scales (monthly, annual) are not computed.


This is controlled in all of function using the **na.rm** argument (similar to many R functions such as mean etc), but usage is slightly different from the standard R usage to allow future versions of the software to have finer control on dealing with missing values.

The treatment of missing values for all function is controlled through the **na.rm=list(na.rm.global=XXXXX)** argument. The current default actions are:
...
We now create some missing values.
```{r}
...
## Computing the Annual Statistics (on calendar and water year)
We compute the plethora of annual statistics with the default action to propogate missing values.
```{r, warning=FALSE}
stat.annual <- compute.Q.stat.annual(
                          Station.Code  =Station.Code,
                          Station.Area  =Station.Area,
...
This has many, many statistics some of which are shown below. Notice that some are now missing (e.g. in 1967) over
and above the missing in the rolling averages for the first year.
```{r}
head(stat.annual$Q.stat.annual[, 1:10])
```

To ignore the missing values, we set the \code{na.rm} parameter:
```{r, warning=FALSE}
...
If you wish missing values to propogate, use:
```{r, warning=FALSE}
...
If you want to propogate missing values, again set the **na.rm** argument:
```{r, warning=FALSE}
...
## Computing a volume frequency analysis
The volume frequence analysis automatically drops missing values:
```{r, warning=FALSE}
vfa.analysis <- compute.volume.frequency.analysis( 
                      Station.Code   =Station.Code, S08HA011,
                      start.year     =start.year, 
