...
Station.Code <- '08HA002'
Station.Name <- 'Cowichan River at Lake Cowichan'
Station.Area <- 594    # square km's

start.year   <- 1941  # when do you want the analysis to start at.
...
# Get the data
flow <- read.csv(file.path("08HA002_Date.csv"), header=TRUE, as.is=TRUE, strip.white=TRUE, skip=1)
head(flow)
...
E.filename <- file.path(report.dir,"08HA002_COWICHAN_STREAMFLOW_SUMMARY.xlsx")
...
HEC.filename <- file.path(report.dir,"COW_FREQ.rpt")
