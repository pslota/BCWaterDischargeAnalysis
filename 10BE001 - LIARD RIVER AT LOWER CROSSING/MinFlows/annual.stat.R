...
Station.Code <- '10BE001'
Station.Name <- 'Liard River at lower crossing'
Station.Area <- 104000    # square km's
...
# Get the data
flow <- read.csv(file.path("10BE001_Date.csv"), header=TRUE, as.is=TRUE, strip.white=TRUE, skip=1)
head(flow)
...
E.filename <- file.path(report.dir,"10BE001_LIARD_STREAMFLOW_SUMMARY.xlsx")
...
HEC.filename <- file.path(report.dir,"Liard_Freq.rpt")
