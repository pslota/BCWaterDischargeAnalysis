...
Station.Code <- '08NJ013'
Station.Name <- 'Slocan River and Crescent Valley'
Station.Area <- 3300    # square km's

start.year   <- 1933  # when do you want the analysis to start at.
end.year     <- 2015  # what is last year with complete data

# Get the data
flow <- read.csv(file.path("08NJ013_Date.csv"), header=TRUE, as.is=TRUE, strip.white=TRUE, skip=1)
# only select param==1
...
E.filename <- file.path(report.dir,"08NJ013_SLOCAN_STREAMFLOW_SUMMARY.xlsx")
...
HEC.filename <- file.path(report.dir,"Slocan_Freq.rpt")
