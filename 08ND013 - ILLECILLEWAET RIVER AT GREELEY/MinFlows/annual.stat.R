...
Station.Code <- '08ND013'
Station.Name <- 'Illecillewaet River at Greeley'
Station.Area <- 1150    # square km's

start.year   <- 1964  # when do you want the analysis to start at.
end.year     <- 2014  # what is last year with complete data

# Get the data
flow <- read.csv(file.path("08ND013_Date.csv"), header=TRUE, as.is=TRUE, strip.white=TRUE, skip=1)
head(flow)
...
E.filename <- file.path(report.dir,"08NJ013_ILLECILLEWAET_STREAMFLOW_SUMMARY.xlsx")
...
HEC.filename <- file.path(report.dir,"GREELEY_FREQ.rpt")
